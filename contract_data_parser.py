...
cwd = os.getcwd()
csv_file = cwd + "/2019_all_Contracts_Full_20190314_2.csv"
...
logger.info("Number of rows: %s " %(row_count(csv_file)))
try:
    # client = MongoClient('mongodb://localhost:27017/')
    client = MongoClient("mongodb+srv://sam:<password>@cluster0-mjstf.mongodb.net/govalert?retryWrites=true",
...
for idx, row in enumerate(iter_contract, start=1):
    logger.info("Processing row number %s " %(idx))
