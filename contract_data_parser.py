...
CONTRACTS_CSV_FILE_NAME = os.getenv("CSV_FILE_NAME")
...
def get_contract_record(csv_fname):
    ...
    ContractRecord = namedtuple('ContractRecord', contracts_column_names)
...
def parse_weather_zones():
    weather_zones = []
    with open(zone_ugc_csv_file, mode='r') as csv_file:
        dialect = csv.Sniffer().sniff(csv_file.read(10024))
        csv_file.seek(0)
...
# def find_ugc_zone_by_county(state_code, county_name, weather_zones)
...
def parse_and_insert_rows(number_rows, collection):
    ...
    for idx, row in enumerate(iter_contract, start=1):
        ...
        batch.append(generate_contract_dict(row, zip_codes, weather_zones))
        if (idx == 1):
            continue
...
number_columns = len(contracts_column_names.split())
number_rows = row_count(contracts_csv_file)
